...
const changeAppointmentStatus = async (appointmentId: string, appointmentStatus: AppointmentStatus, user: IRequestUser) => {
    ...
    if (appointmentData.status === AppointmentStatus.COMPLETED || appointmentData.status === AppointmentStatus.CANCELED) {
        throw new AppError(status.BAD_REQUEST, "Appointment already completed or cancelled");
    }
    ...
    if (user?.role === Role.DOCTOR) {
        ...
        const isScheduleToInProgress = appointmentData.status === AppointmentStatus.SCHEDULED && appointmentStatus === AppointmentStatus.INPROGRESS;
        const isInProgressToCompleted = appointmentData.status === AppointmentStatus.INPROGRESS && appointmentStatus === AppointmentStatus.COMPLETED;
        const isScheduleToCancelled = appointmentData.status === AppointmentStatus.SCHEDULED && appointmentStatus === AppointmentStatus.CANCELED;

        if (!isScheduleToInProgress && !isInProgressToCompleted && !isScheduleToCancelled) {
            throw new AppError(status.BAD_REQUEST, "Invalid status change for doctor");
        }
        ...
        if (appointmentData.status !== AppointmentStatus.SCHEDULED || appointmentStatus !== AppointmentStatus.CANCELED) {
            throw new AppError(status.BAD_REQUEST, "Invalid status change for patient");
        }
    }
